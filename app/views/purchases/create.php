...
function updateItem(tempId, field, value) {
    ...
    // Recalculate nett if buy_price changes
    if (field === 'buy_price') {
...
function onPkgModalInput(inputEl, level) {
    ...
    if (nettInfoEl) {
        if (ppn > 0 || diskonVal > 0) {
            const ppnAmt = Math.round(buy * ppn / 100);
            const diskonAmt = diskonMode === 'pct' ? Math.round(buy * diskonVal / 100) : Math.round(diskonVal);
            nettInfoEl.innerHTML = '<span style="color:var(--text-muted);">Modal: Rp' + Math.round(buy).toLocaleString('id-ID') + '</span>'
                + (ppn > 0 ? ' <span style="color:var(--warning);">+PPN: Rp' + ppnAmt.toLocaleString('id-ID') + '</span>' : '')
                + (diskonVal > 0 ? ' <span style="color:var(--success);">\u2212Diskon: Rp' + diskonAmt.toLocaleString('id-ID') + '</span>' : '')
                + ' \u2192 <strong style="color:var(--info);">Nett: Rp' + Math.round(nett).toLocaleString('id-ID') + '</strong>';
        } else { nettInfoEl.innerHTML = ''; }
    }

    if (marginEl) {
        marginEl.querySelector('.margin-retail-text').innerHTML = formatMarginWithProfit('Ecer', nett, ret);
        marginEl.querySelector('.margin-wholesale-text').innerHTML = formatMarginWithProfit('Grosir', nett, who);
    }
    if (typeof PackagingPriceSync !== 'undefined') {
...
function calcItemNett(buy, ppn_pct, diskon_mode, diskon_value) {
    buy = parseFloat(buy) || 0;
    const ppn_amt = Math.round(buy * ((parseFloat(ppn_pct) || 0) / 100));
    const diskon_amt = diskon_mode === 'pct'
        ? Math.round(buy * ((parseFloat(diskon_value) || 0) / 100))
        : (Math.round(parseFloat(diskon_value) || 0));
...
function buildNettInfo(item) {
    ...
    const ppn_amt = Math.round(buy * (ppn / 100));
    const diskon_amt = diskonMode === 'pct' ? Math.round(buy * (diskon / 100)) : Math.round(diskon);
    ...
    if (ppn > 0) html += ` <span style="color:var(--warning);">+PPN(${ppn}%): Rp${ppn_amt.toLocaleString('id-ID')}</span>`;
    if (diskon > 0) html += ` <span style="color:var(--success);">−Diskon: Rp${diskon_amt.toLocaleString('id-ID')}</span>`;
    html += ` → <strong style="color:var(--info);">Nett: Rp${Math.round(nett).toLocaleString('id-ID')}</strong>`;
    html += `</div>`;
    return html;
}

/** Update PPN/Diskon for an item and refresh nett + margin */
function updateItemPpnDiskon(tempId, type, value) {
    const item = purchaseItems.find(i => i.id == tempId);
    if (!item) return;
    if (type === 'ppn') item.ppn_pct = parseFloat(value) || 0;
    else if (type === 'mode') item.diskon_mode = value;
    else if (type === 'diskon') item.diskon_value = parseFloat(value) || 0;
...
function buildPkgMiniSummaryHtml(item) {
    ...
    const rows = otherPkgs.map(pkg => {
        ...
        return '<div style="background:rgba(0,0,0,0.12);border-radius:6px;padding:6px 8px;margin-bottom:4px;border:1px solid rgba(255,255,255,0.04);">'
            ...
            + '<div style="background:rgba(0,0,0,0.2);padding:4px 6px;border-radius:4px;"><div style="color:var(--text-muted);font-size:8px;">Modal Nett</div><div style="font-weight:600;font-size:10px;">' + (nb > 0 ? 'Rp'+nb.toLocaleString('id-ID') : '—') + '</div></div>'
    ...
    return '<div style="margin-top:8px;border-top:1px dashed rgba(255,255,255,0.08);padding-top:8px;"><div style="font-size:9px;color:var(--info);font-weight:600;margin-bottom:6px;"><i class="bi bi-info-circle-fill"></i> Info harga kemasan lain (margin vs Nett)</div>' + rows + '</div>';
}
        ...
        html += `
            <div style="background:var(--surface-1);border-radius:var(--radius-lg);padding:16px;margin-bottom:12px;border:1px solid var(--border-color);position:relative;">
                ...
                <div style="background:rgba(0,0,0,0.15);padding:10px;border-radius:var(--radius-sm);border:1px solid rgba(255,255,255,0.05);">
                    ...
                    <div style="font-size:9px;color:var(--text-muted);margin-bottom:2px;"><i class="bi bi-info-circle"></i> Margin dihitung dari Harga Nett</div>
                ...
                <div style="background:rgba(76,201,240,0.06);border:1px dashed rgba(76,201,240,0.3);border-radius:4px;padding:7px;margin-bottom:8px;">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 2fr;gap:5px;margin-bottom:5px;">
                        <div>
                            ...
                            <input type="number" class="bulk-ppn form-control-dark" style="width:100%;font-size:10px;padding:4px;" value="0" min="0" max="100" placeholder="0" oninput="calcBulkMargin(this)">
                        </div>
                        <div>
                            ...
                            <div style="display:flex;gap:3px;">
                                ...
                                <input type="number" class="bulk-diskon-value form-control-dark" style="flex:1;font-size:10px;padding:4px;" value="0" min="0" placeholder="0" oninput="calcBulkMargin(this)">
                            </div>
                        </div>
                    </div>
