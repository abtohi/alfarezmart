                ...
                <div class="purchases-create-step1-row" style="display:flex; gap:12px; flex-direction:column;">
                    ...
                    <div>
                        ...
                        <div style="display:flex; gap:8px;">
                            <button type="button" class="btn-outline-custom" id="btnPhotoCam" onclick="triggerCameraCapture(event)" style="flex:1; padding:10px 4px; font-size:12px; height:44px; display:inline-flex; align-items:center; justify-content:center; gap:6px; cursor:pointer; margin:0;">
                                <i class="bi bi-camera"></i> Kamera
                            </button>
                            <button type="button" class="btn-outline-custom" id="btnPhotoGal" onclick="triggerGalleryPicker(event)" style="flex:1; padding:10px 4px; font-size:12px; height:44px; display:inline-flex; align-items:center; justify-content:center; gap:6px; cursor:pointer; margin:0;">
                                <i class="bi bi-image"></i> Galeri
                            </button>
                        </div>

                        <!-- Thumbnail Preview Card if Photo Selected -->
                        <div id="invoicePhotoPreviewCard" style="display:none; margin-top:8px; padding:10px 12px; background:var(--bg-input, rgba(255,255,255,0.03)); border:1px solid var(--border-color); border-radius:var(--radius-md, 8px); align-items:center; gap:12px;">
                            <div style="position:relative; width:48px; height:48px; border-radius:6px; overflow:hidden; border:1px solid var(--border-color); flex-shrink:0; background:#000; display:flex; align-items:center; justify-content:center;">
                                <img id="invoicePhotoThumbnail" src="" alt="Thumbnail Invoice" style="width:100%; height:100%; object-fit:cover; display:block;">
                            </div>
                            <div style="flex:1; min-width:0;">
                                <div style="font-size:12px; font-weight:600; color:var(--text-primary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">Foto Invoice Terpilih</div>
                                <div style="font-size:10px; color:var(--text-muted); display:flex; align-items:center; gap:4px; margin-top:2px;">
                                    <i class="bi bi-check-circle-fill" style="color:var(--color-success, #10b981);"></i> Siap di-scan atau disimpan
                                </div>
                            </div>
                            <div style="display:flex; gap:6px; flex-shrink:0;">
                                <button type="button" class="btn-outline-custom" onclick="reopenPhotoPreview()" style="padding:6px 10px; font-size:11px; height:32px; border-radius:6px;" title="Atur atau putar kembali">
                                    <i class="bi bi-pencil-square"></i> Atur
                                </button>
                                <button type="button" class="btn-outline-custom" onclick="removeInvoicePhoto()" style="padding:6px 8px; font-size:11px; height:32px; border-radius:6px; color:var(--color-danger, #ef4444); border-color:rgba(239,68,68,0.3);" title="Hapus foto">
...
function handlePhotoSelect(e, isCamera) {
    ...
    const loadImageToCanvas = (src, isBlob = false) => {
        ...
        img.onerror = function() {
            ...
            // Fallback if blob loading failed: try FileReader
            if (isBlob) {
...
function removeInvoicePhoto() {
    ...
    showToast('Foto invoice dihapus', 'info');
}
...
/**
 * Auto-detect and preserve custom price flags (buy_custom & sell_custom) across packaging levels.
 * For Level > 1, a packaging is marked custom if:
 * 1. It was already explicitly flagged as custom (buy_custom === true / sell_custom === true), OR
 * 2. Its buy/sell price deviates from the proportional value calculated from Level 1.
 * This guarantees custom checkboxes remain checked when loaded via AI scan, search, barcode, or edit.
 */
