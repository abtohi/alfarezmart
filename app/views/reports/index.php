...
<div class="page-section">
    ...
    <div style="background:var(--surface-1); border-radius:var(--radius-lg); padding:16px; margin-bottom:20px; border:1px solid var(--border-color);">
        <form action="<?= BASE_URL ?>reports" method="GET" style="display:flex; gap:12px; align-items:center;">
            <input type="date" name="date" class="form-control-dark" value="<?= htmlspecialchars($date) ?>" style="flex:1;" onchange="this.form.submit()">
    ...
    <div style="display:grid; grid-template-columns:1fr; gap:12px; margin-bottom:20px;">
        ...
        <div style="background:var(--success-bg); border:1px solid var(--success); border-radius:var(--radius-lg); padding:16px; display:flex; align-items:center; gap:16px;">
            ...
            <div>
                ...
                <div style="font-size:24px; font-weight:800; color:var(--text-primary);"><?= formatRupiah($salesStats['revenue']) ?></div>
                <div style="font-size:11px; color:var(--text-muted); margin-top:2px;">Dari <?= $salesStats['transactions'] ?> transaksi kasir</div>
            </div>
        </div>
        ...
        <div style="background:var(--danger-bg); border:1px solid var(--danger); border-radius:var(--radius-lg); padding:16px; display:flex; align-items:center; gap:16px;">
            ...
            <div>
                ...
                <div style="font-size:24px; font-weight:800; color:var(--text-primary);"><?= formatRupiah($purchaseStats['expense']) ?></div>
                <div style="font-size:11px; color:var(--text-muted); margin-top:2px;">Ke <?= $purchaseStats['transactions'] ?> supplier/nota</div>
            </div>
        </div>
        ...
        <div style="background:var(--info-bg); border:1px solid var(--info); border-radius:var(--radius-lg); padding:16px; display:flex; align-items:center; gap:16px;">
            ...
            <div>
                ...
                <div style="font-size:24px; font-weight:800; color:var(--text-primary);"><?= formatRupiah($salesStats['gross_profit']) ?></div>
                <div style="font-size:11px; color:var(--text-muted); margin-top:2px;">Margin dari <?= $salesStats['items_sold'] ?> item terjual</div>
            </div>
        </div>
