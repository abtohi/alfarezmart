...
<input type="hidden" id="csrfToken" value="<?= $__globalCsrf ?>">
    ...
    <aside class="desktop-sidebar" id="desktopSidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="<?= BASE_URL ?>settings/error-logs" class="sidebar-item <?= ($activeNav ?? '') === 'error_logs' ? 'active' : '' ?>" title="Lihat &amp; Salin Log Error">
                <i class="bi bi-bug"></i>
                <span>Error Log Catcher</span>
            </a>
            <a href="<?= BASE_URL ?>settings" class="sidebar-item <?= ($activeNav ?? '') === 'settings' ? 'active' : '' ?>">
    ...
    <script>
    const APP_VERSION = '25.23'; // Update this to force client reloads
