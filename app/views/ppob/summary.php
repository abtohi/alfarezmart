    ...
    function renderSellerTransactionHistory(trxList) {
        ...
        let rowsHtml = trxList.map(t => {
            ...
            if (t.duration_seconds !== null && t.duration_seconds !== undefined) {
                ...
                if (speedVal > 900) {
                    speedBadge = `<span style="background:rgba(107,114,128,0.12);color:var(--text-secondary);font-size:9.5px;padding:2px 6px;border-radius:4px;font-weight:600;white-space:nowrap;" title="Status diperbarui melalui cek status berkala (>15m)"><i class="bi bi-clock-history me-1"></i>>15m (Sync)</span>`;
                } else {
