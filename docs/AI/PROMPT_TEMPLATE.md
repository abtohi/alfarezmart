...
- Tambahkan fitur berupa icon untuk mengaktifkan offline mode secara full, sehingga ketika fitur ini ditekan, maka di belakang layar akan secara otomatis mendownload semua database dan menyimpannya di local, baik di laptop, android, maupun IOS, harus support bisa dibuka disemua perangkat tersebut. User bisa mengatur kapan proses sinkronisasi berjalan secara manual (dengan menekan tombol sync), atau bisa juga diatur secara otomatis (ketika app online). Jika user mengatur sinkronisasi secara otomatis, maka ketika app online maka akan terjadi sinkronisasi, dan ketika app offline, maka tidak akan terjadi sinkronisasi. Pastikan semua proses sinkronisasi ini bisa berjalan dengan cepat dan tidak mengganggu pengalaman pengguna, pastikan juga tidak ada data yang hilang atau rusak selama proses sinkronisasi. Jika user sudah login dan berada di dalam aplikasi, user tetap bisa menggunakan fitur-fitur yang tersedia tanpa harus login kembali. Saat ini sepertinya fitur ini masih belum sepenuhnya berjalan, ketika saya menyimpan data secara offline masih gagal, selalu diarahkan ke beranda. Aku mau kamu mengatasi ini semua, dan aktifkan count notif di icon sync, agar tau ada berapa item yang belum tersinkron ke server.
